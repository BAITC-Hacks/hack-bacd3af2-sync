...
- `GET /api/metrics` пока отдаёт демо-метрики; метрики ML-части лежат в `ml/models/turbine_N/metrics.json`.
...
- Отдавать в `GET /api/metrics` метрики ML-части (`ml/models/turbine_N/metrics.json`) вместо демо-значений.
