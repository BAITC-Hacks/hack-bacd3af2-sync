...
**В текущей ветке `ml` приложение запускается в демонстрационном режиме.** Подключение реальной модели подготовлено в отдельной ветке `backend`. Ниже — результаты проверки настоящей обученной модели, а не демонстрационные числа интерфейса.
